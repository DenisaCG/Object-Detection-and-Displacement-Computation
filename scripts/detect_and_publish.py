...
class Object_Detector:
    def __init__(self, opt):
        self.opt = opt
        self.pub = rospy.Publisher('/eblan/camera_node/image/detected', Image, queue_size=10)
        
    def callback(self, data):
        bridge = CvBridge()
        cv_image = bridge.imgmsg_to_cv2(data, "bgr8")
    
        img_save_path = "test/frame_images/frame.jpg"
        if os.path.exists(img_save_path):
            os.remove(img_save_path)
            print("Ready for new frame")
        cv2.imwrite(img_save_path, cv_image)
    ...
    def subscriber(self):
        rospy.Subscriber("/eblan/camera_node/image/raw", Image, self.callback)
...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_argument('--source', type=str, default='test/frame_images', help='source')  # file/folder, 0 for webcam
